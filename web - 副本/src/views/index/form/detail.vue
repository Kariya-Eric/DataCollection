<template>
  <div>
    ...
    <el-card shadow="always" class="app-card" style="margin-top: 16px">
      ...
      <dc-table @selection-change="onSelectChange" :data="dataSource" :columns="columns" :pagination="ipagination" selection="selection" @change="loadData">
        ...
        <template slot="enabled" slot-scope="scope">
          <el-switch v-model="scope.row.enabledFlag" :inactive-value="0" :active-value="1" @change="val => enableFormCollection(val, scope.row)" />
        </template>
    ...
    <add-form-dialog ref="modalForm" @refresh="loadData" :collection="collectionDetail" :categorys="listCategories" />
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { slot: 'action', label: '操作', width: '330px' }
      ],
      collectionDetail: {},
      url: {
        list: '',
