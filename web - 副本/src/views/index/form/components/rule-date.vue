...
export default {
  ...
  computed: {
    dateList() {
      return this.drawingList.filter(
        (item) => item.__config__.tag == "el-date-picker"
      );
    },
  },
  ...
  methods: {
    ...
    validate() {
      ...
      for (let i = 0; i < this.dateRule.length; i++) {
        ...
        if (
          this.dateRule[i].left[0].type == "" ||
          this.dateRule[i].left[0].value == ""
        ) {
          flag = false;
          break;
        }
        if (
          this.dateRule[i].right[0].type == "" ||
          this.dateRule[i].right[0].value == ""
        ) {
          flag = false;
          break;
        }
