<template>
  <div class="right-board">
    <a-tabs v-model="currentTab" class="center-tabs">
      <a-tab-pane label="组件属性" name="field" />
      <a-tab-pane label="表单属性" name="form" />
    </a-tabs>
    <div class="field-box">
      <a class="document-link" target="_blank" :href="documentLink" title="查看组件文档">
        ...
        <!-- 组件属性 -->
        <a-form v-show="currentTab==='field' && showField" size="small" laba-width="90px">
          <a-form-item v-if="activeData.__config__.changeTag" label="组件类型">
            <a-select
              ...
              @change="tagChange"
            >
              <a-option-group v-for="group in tagList" :key="group.label" :label="group.label">
                <a-option
          ...
          <a-form-item v-if="activeData.__vModel__!==undefined" label="字段名">
          ...
          <a-form-item v-if="activeData.__config__.span!==undefined" label="表单栅格">
            <a-slider v-model="activeData.__config__.span" :max="24" :min="1" :marks="{12:''}" @change="spanChange" />
          </a-form-item>
          <a-form-item v-if="activeData.__config__.layout==='rowFormItem'&&activeData.gutter!==undefined" label="栅格间隔">
            <a-input-number v-model="activeData.gutter" :min="0" placeholder="栅格间隔" />
          </a-form-item>
          <a-form-item v-if="activeData.__config__.layout==='rowFormItem'&&activeData.type!==undefined" label="布局模式">
          ...
          <a-form-item v-if="activeData.__config__.labelWidth!==undefined" label="标签宽度">
          ...
          <a-form-item v-if="activeData.__config__.tag==='a-checkbox-group'" label="至少应选">
            <a-input-number
          ...
          <a-form-item v-if="activeData.__config__.tag==='a-checkbox-group'" label="最多可选">
            <a-input-number
          ...
          <a-form-item v-if="activeData.__slot__&&activeData.__slot__.prepend!==undefined" label="前缀">
            <a-input v-model="activeData.__slot__.prepend" placeholder="请输入前缀" />
          </a-form-item>
          <a-form-item v-if="activeData.__slot__&&activeData.__slot__.append!==undefined" label="后缀">
          ...
          <a-form-item v-if="activeData['prefix-icon']!==undefined" label="前图标">
            <a-input v-model="activeData['prefix-icon']" placeholder="请输入前图标名称">
              <a-button slot="append" icon="a-icon-thumb" @click="openIconsDialog('prefix-icon')">
                选择
              </a-button>
            </a-input>
          </a-form-item>
          <a-form-item v-if="activeData['suffix-icon'] !== undefined" label="后图标">
            <a-input v-model="activeData['suffix-icon']" placeholder="请输入后图标名称">
              <a-button slot="append" icon="a-icon-thumb" @click="openIconsDialog('suffix-icon')">
                选择
              </a-button>
            </a-input>
          </a-form-item>
          ...
          >
            <a-input v-model="activeData['icon']" placeholder="请输入按钮图标名称">
              <a-button slot="append" icon="a-icon-thumb" @click="openIconsDialog('icon')">
                选择
              </a-button>
            </a-input>
          </a-form-item>
          <a-form-item v-if="activeData.__config__.tag === 'a-cascader'" label="选项分隔符">
          ...
          <a-form-item v-if="activeData.height!==undefined" label="组件高度">
            <a-input-number v-model="activeData.height" placeholder="高度" @input="changeRenderKey" />
          </a-form-item>
          <a-form-item v-if="isShowStep" label="步长">
          ...
          <a-form-item v-if="activeData.__config__.tag === 'a-input-number'" label="按钮位置">
          ...
          <a-form-item v-if="activeData.__config__.fileSize !== undefined" label="文件大小">
          ...
          >
            <a-select v-model="activeData.type" :style="{ width: '100%' }">
              <a-option label="primary" value="primary" />
              <a-option label="success" value="success" />
              <a-option label="warning" value="warning" />
              <a-option label="danger" value="danger" />
              <a-option label="info" value="info" />
          ...
          <template v-if="['a-cascader'].indexOf(activeData.__config__.tag) > -1">
            <a-divider>选项</a-divider>
            <a-form-item label="数据类型">
              <a-radio-group v-model="activeData.__config__.dataType" size="small">
            ...
            <template v-if="activeData.__config__.dataType === 'dynamic'">
              <a-form-item label="接口地址">
                ...
                >
                  <a-select
                    ...
                    :style="{width: '85px'}"
                  >
                    <a-option label="get" value="get" />
                    <a-option label="post" value="post" />
                    <a-option label="put" value="put" />
                    <a-option label="delete" value="delete" />
                  </a-select>
                </a-input>
              </a-form-item>
              ...
              <a-form-item label="子级键名">
                <a-input v-model="activeData.props.props.children" placeholder="请输入子级键名" />
              </a-form-item>
            </template>
          ...
          <a-form-item v-if="activeData.__config__.optionType !== undefined" label="选项样式">
            <a-radio-group v-model="activeData.__config__.optionType">
              <a-radio-button label="default">
                默认
              </a-radio-button>
              <a-radio-button label="button">
                按钮
              </a-radio-button>
            </a-radio-group>
          </a-form-item>
          ...
          <a-form-item v-if="activeData.__config__.showLabel !== undefined
            && activeData.__config__.labelWidth !== undefined" label="显示标签"
          >
            <a-switch v-model="activeData.__config__.showLabel" />
          </a-form-item>
          <a-form-item v-if="activeData.branding !== undefined" label="品牌烙印">
          ...
          <a-form-item v-if="activeData.__config__.tag === 'a-color-picker'" label="颜色格式">
            <a-select
          ...
          <a-form-item
            v-if="activeData.size !== undefined &&
              (activeData.__config__.optionType === 'button' ||
                ...
                activeData.__config__.tag === 'a-color-picker' ||
          ...
          <a-form-item v-if="activeData.__config__.tag === 'a-input-number'" label="严格步数">
            <a-switch v-model="activeData['step-strictly']" />
          </a-form-item>
          <a-form-item v-if="activeData.__config__.tag === 'a-cascader'" label="任选层级">
            <a-switch v-model="activeData.props.props.checkStrictly" />
          </a-form-item>
          <a-form-item v-if="activeData.__config__.tag === 'a-cascader'" label="是否多选">
            <a-switch v-model="activeData.props.props.multiple" />
          </a-form-item>
          <a-form-item v-if="activeData.__config__.tag === 'a-cascader'" label="展示全路径">
            <a-switch v-model="activeData['show-all-levels']" />
          </a-form-item>
          <a-form-item v-if="activeData.__config__.tag === 'a-cascader'" label="可否筛选">
          ...
          <a-form-item v-if="activeData.__config__.tag === 'a-upload'" label="多选文件">
          ...
          <a-form-item v-if="activeData.__config__.tag === 'a-select'" label="是否多选">
          ...
          <template v-if="Array.isArray(activeData.__config__.regList)">
            ...
            <div style="margin-left: 20px">
              <a-button icon="a-icon-circle-plus-outline" type="text" @click="addReg">
                添加规则
              </a-button>
            </div>
          </template>
        ...
        <!-- 表单属性 -->
        <a-form v-show="currentTab === 'form'" size="small" laba-width="90px">
          ...
          <a-form-item label="标签宽度">
            <a-input v-model.number="formConf.labelWidth" type="number" placeholder="请输入标签宽度" />
          </a-form-item>
