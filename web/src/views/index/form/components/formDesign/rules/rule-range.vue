<template>
  <div class="typeDiv">
    <div class="buttonDiv">
      <el-button icon="el-icon-circle-plus-outline" plain size="small" @click="add(0)"> 且条件 </el-button>
      <el-button icon="el-icon-circle-plus-outline" plain size="small" @click="add(1)"> 或条件 </el-button>
    </div>
    <div v-for="(range, index) in dataRangeVal" :key="index">
      ...
      <div class="innerDiv">
        ...
        <el-row v-for="(left, leftIndex) in range.left" :key="'left' + leftIndex">
          <el-col :span="3">
            <span v-if="leftIndex == 0"> 公式左侧 </span>
            <el-select v-else v-model="left.operator" placeholder="运算符" style="width: 100%" @change="val => changeLeftOpt(val, leftIndex, index)">
              <el-option :label="item" :value="item" v-for="item in calculateList" :key="'left' + item" />
            </el-select>
          </el-col>
          <el-col :span="8">
            <el-select placeholder="请选择" v-model="left.type" style="width: 100%" @change="val => changeLeftType(val, leftIndex, index)">
          ...
          <el-col :span="8" v-if="left.type != ''">
            ...
            <el-input v-else v-model="left.value" placeholder="请输入" @input="val => changeLeftValue(val, leftIndex, index)" />
          </el-col>
          <el-col :span="2">
            ...
            <div v-else class="rowIconDel">
              <i class="el-icon-remove-outline" @click="delLeft(leftIndex, index)" />
            </div>
          </el-col>
        </el-row>
        ...
        <el-row>
          ...
          <el-col :span="8">
            <el-select placeholder="请选择" v-model="range.operator" @change="val => changeOpt(val, index)">
              <el-option v-for="(opt, optIndex) in operatorList" :key="optIndex" :label="opt" :value="opt" />
            </el-select>
          </el-col>
        </el-row>

        <el-row v-for="(right, rightIndex) in range.right" :key="'right' + rightIndex">
          <el-col :span="3">
            <span v-if="rightIndex == 0"> 公式右侧 </span>
            <el-select v-else v-model="right.operator" placeholder="运算符" style="width: 100%" @change="val => changeRightOpt(val, rightIndex, index)">
              <el-option :label="item" :value="item" v-for="item in calculateList" :key="'right' + item" />
            </el-select>
          </el-col>
          <el-col :span="8">
            <el-select placeholder="请选择" v-model="right.type" style="width: 100%" @change="val => changeRightType(val, rightIndex, index)">
              <el-option label="表单字段" value="field" />
              <el-option label="固定值" value="fixed" />
            </el-select>
          </el-col>
          <el-col :span="8" v-if="right.type != ''">
            ...
            <el-input v-else v-model="right.value" placeholder="请输入" @input="val => changeRightValue(val, rightIndex, index)" />
          </el-col>
          <el-col :span="2">
            ...
            <div v-else class="rowIconDel">
              <i class="el-icon-remove-outline" @click="delRight(rightIndex, index)" />
            </div>
          </el-col>
        </el-row>
      </div>
