<template>
  <el-drawer :append-to-body="true" :visible="visible" :show-close="false" @opened="drawerOpen" :size="1200">
    <div slot="title" class="card-title">
      <span>资源授权</span>
      <div class="card-title-button">
        <el-button type="primary" @click="savePermission">保存</el-button>
        <el-button name="返回" @click="close"><svg-icon icon-class="返回" />返回</el-button>
      </div>
    </div>
    ...
    <el-row :gutter="24">
      ...
      <el-col :span="18">
        <el-table size="small" :data="sysMethodDataSource" :border="true" :loading="tableLoading">
          <el-table-column type="selection" width="55" align="center">
            <template slot-scope="scope">
              <el-checkbox :value="scope.row.isSelected == '1'" @change="val => changeChecked(scope.row, val)" />
            </template>
          </el-table-column>
...
<script>
import { getAllMenuByRoleCode, getAllMethodByRoleCode, saveRoleAuth } from '@/api/system'
export default {
  ...
  methods: {
    ...
    savePermission() {
      let arrMenuAlias = this.$refs.menuTree.getCheckedNodes().map(node => node.alias)
      let arrMethodAlias = this.sysMethods.filter(data => data.isSelected == '1').map(data => data.alias)
      let param = {
        arrMenuAlias,
        arrMethodAlias,
