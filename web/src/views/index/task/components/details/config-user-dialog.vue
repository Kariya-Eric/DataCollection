<template>
  <el-dialog title="配置人员" @close="close" width="30%" :visible="visible" :append-to-body="true" v-if="visible">
    <el-form size="small" :model="configForm" ref="configForm" label-width="80px" :rules="configFormRules">
...
export default {
  ...
  watch: {
    'configForm.responsibleUser'(newVal) {
      if (newVal != '') {
        this.configForm.responsibleUserName = this.userList.find(item => item.id == newVal).name
      }
    },
    'configForm.fillUser'(newVal) {
      if (newVal != '') {
        this.configForm.fillUserName = this.userList.find(item => item.id == newVal).name
      }
    }
  },
  mounted() {
