<template>
  <div>
    <div class="echart" ref="professorTeamAmount"></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    initEcharts() {
      const parentNode = this.$refs.professorTeamAmount.parentNode.parentNode
      // 基本柱状图
      const option = {
        ...
        series: [
          ...
          { type: 'bar', name: '专任教师总数', data: this.totalData, stack: 'total' }
        ]
      }
      const myChart = echarts.init(this.$refs.professorTeamAmount)
