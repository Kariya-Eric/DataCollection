<template>
  <div>
    <div class="echart" ref="talentTeamAmount"></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    initEcharts() {
      const parentNode = this.$refs.talentTeamAmount.parentNode.parentNode
      // 基本柱状图
      const option = {
        grid: { top: '18%', left: '16%', right: '8%', bottom: '12%' },
        tooltip: { trigger: 'axis' },
        title: { text: '高层次人才党员数量' },
        yAxis: { type: 'value' },
        xAxis: { type: 'category', data: this.xAxis },
        series: [
          { type: 'bar', name: '党员数', data: this.teamData, stack: 'total' },
          { type: 'bar', name: '高层次人才总数', data: this.totalData, stack: 'total' }
        ]
      }
      const myChart = echarts.init(this.$refs.talentTeamAmount)
