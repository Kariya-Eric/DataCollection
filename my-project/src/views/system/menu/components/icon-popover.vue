<template>
  <div style="width: 900px">
    <a-tabs>
      ...
      <a-tab-pane tab="其他" key="6">
        <ul>
          <li v-for="icon in icons.customIcons" :key="icon">
            <dc-icon :type="icon" :title="icon" @click="chooseDcIcon(icon)" :class="{ active: activeIndex === icon }" />
          </li>
        </ul>
        <a-divider />
        <ul>
...
const customIcons = [
  ...
  'icon-dc_import',
  'icon-dc_copy',
  'icon-dc_datacollection',
  'icon-dc_reject',
  'icon-dc_edit',
  'icon-dc_back',
  'icon-dc_announce',
  'icon-dc_waiting_submit',
  'icon-dc_empty',
  'icon-dc_system_manage',
  'icon-dc_view',
  'icon-dc_batch_config',
  'icon-dc_save',
  'icon-dc_arrow',
  'icon-dc_waiting_config',
  'icon-dc_waiting_audit'
]
...
export default {
  ...
  methods: {
    ...
    chooseDcIcon(icon) {
      ...
      this.choosedIcon = icon.substring(5)
    },
    chooseIcon(icon) {
      this.activeIndex = icon
