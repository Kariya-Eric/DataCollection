...
import { DCVXETypes } from '../dcvxeTypes'
...
import { cloneObject, getVmParentByName, randomString, simpleDebounce } from '../utils/util'
...
export default {
  ...
  watch: {
    ...
    columns: {
      ...
      handler(columns) {
        ...
        if (dragSort) {
          let width = 40
          let col = { type: DCVXETypes.rowDragSort, title: '', width, fixed: 'left', align: 'center', cellRender: { name: DCVXETypes._prefix + DCVXETypes.rowDragSort } }
          if (dragSortColumn) {
  ...
  methods: {
    ...
    _remove(rows) {
      ...
      const { afterFullData, tableFullData, tableSourceData, editStore, checkboxOpts, selection, isInsertByRow, scrollYLoad } = xTable
  ...
  render(h) {
    return h(
      ...
      [this.renderToolbar(h), this.renderToolbarAfterSlot(), this.renderVxeGrid(h)]
    )
  },
  beforeDestroy() {
