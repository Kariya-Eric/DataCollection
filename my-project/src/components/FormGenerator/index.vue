<template>
  <a-layout id="components-layout-demo-fixed-sider">
    <a-layout-sider :width="265" theme="light" :style="{ maxHeight: '960px', left: 12 }">
      <div class="left-main">
    ...
    <a-layout-sider :width="330" theme="light" :style="{ maxHeight: '960px', right: 12 }">
      <right-panel
        id="right-panel"
  ...
  methods: {
    ...
    validateFields() {
      ...
      if (valid.empty.length == 0 && valid.unique.length == 0 && valid.illegal.length == 0) {
        ...
        return `字段名非法，请使用字母下划线数字的组合且数字下划线不能在第一位：【${valid.illegal.join(',')}】`
      } else if (valid.empty.length != 0 && valid.unique.length != 0 && valid.illegal.length == 0) {
        return `字段名为空：【${valid.empty.join(',')}】;` + '\n' + `字段名重复:【${valid.unique.join(',')}】`
      } else if (valid.empty.length != 0 && valid.unique.length == 0 && valid.illegal.length != 0) {
        return `字段名为空：【${valid.empty.join(',')}】;` + '\n' + `字段名非法，请使用字母下划线数字的组合且数字下划线不能在第一位：【${valid.illegal.join(',')}】`
      } else if (valid.empty.length == 0 && valid.unique.length != 0 && valid.illegal.length != 0) {
        return `字段名重复:【${valid.unique.join(',')}】;` + '\n' + `字段名非法，请使用字母下划线数字的组合且数字下划线不能在第一位：【${valid.illegal.join(',')}】`
      } else {
        return (
