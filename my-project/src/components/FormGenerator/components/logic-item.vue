<template>
  <div class="item">
    <a-select v-model="term" style="width: 25%" @change="changeTerm">
      <a-select-option v-for="(item, index) in options" :key="index" :value="item.value">{{ item.label }}</a-select-option>
    </a-select>
    <a-select style="width: 15%" v-model="calFlag" :disabled="calFlagList.length == 0" @change="changeCalFlag">
    ...
    <a-date-picker :disabled="calFlagList.length == 0" v-else-if="inputType == 2" v-model="termVal" @change="changeTermVal" style="width: 55%" />
    ...
    <a-input v-else v-model="termVal" style="width: 55%" @change="changeTermVal" :disabled="calFlagList.length == 0" />
  </div>
</template>
